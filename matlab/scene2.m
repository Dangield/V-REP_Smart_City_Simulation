...
%Destination
destination = [15, 10, 16, 1, 2, 7, 8, 6];
...
while ~reachAllGoal
    ...
    for k = 1:numCars              
        ...
        if currPermission            
            ...
            vrepSim.cars{k}.setSpeed(0);
        end
        vrepSim.cars{k}.setAngle(steeringAngle);


        %Check if the vehicle reaches the goal
        reachGoal(k) =  (pdist([goalPose(k,1:2); currentPose(k,1:2)],'euclidean') < 1);
        if reachGoal(k)
            vrepSim.removeCar(k);        
            %Remove car
            worldMap.deleteAgents(k); 
            deletedCars = [deletedCars, k];
        end
    end
